...
const badgeVariants = tv({
  ...
  variants: {
    ...
    variant: {
      ...
      outline: 'border bg-background px-2.5 py-0.5',
      solid: 'border px-2.5 py-0.5',
    },
  },
});
...
const Badge = React.forwardRef<HTMLSpanElement, BadgeProps>(
  ({ className, asChild = false, intent, variant, ...props }, ref) => {
    const Comp = asChild ? Slot : 'span';
    return (
      <Comp
        className={cn(badgeVariants({ className, intent, variant }))}
